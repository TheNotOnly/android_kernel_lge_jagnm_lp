...
cd split_img;
kernel="zImage";               echo "kernel = $kernel";
if [ "$1" = "--original" ]; then
...
echo "Building image...";
echo " ";
$bin/$arch/mkbootimg --kernel "split_img/$kernel" --ramdisk "$ramdisk" $second --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff $secondoff --tags_offset $tagsoff $dtb -o boot.img;
if [ ! $? -eq "0" ]; then
  abort;
  exit 1;
fi;
